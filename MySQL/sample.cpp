...
#define USER "root"
#define PASSWORD "Kick2poo"
#define DATABASE "dictionaryStudyTool"
...
int main(int argc, const char *argv[]) {
  ...
  try {
    ...
    cout << "Executing the Query: \"SELECT * FROM entries\" .." << endl;

    /* run a query which returns exactly one result set */
    res = stmt -> executeQuery ("SELECT * FROM entries WHERE word = 'apple'");

    cout << "Retrieving the result set .." << endl;

    /* retrieve the data from the result set and display on stdout */
    retrieve_data_and_print (res, NUMOFFSET, 1, string("name"));
